...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL(CREATE_TABLE_QUERY);            // YUKARIYA PRİMARY KEY DEN SONRA AUTOINCREMENT EKLEYEREK OTOMATİK ARTMASINI SAĞLAYABİLİRİZ.
        this.database=db;
    }
    ...
    public boolean insertUser(String userName, String name, String email, String password) {
        ...
        Long result=database.insert(TABLE_NAME,null,value);   // LONG DEĞER DÖNDÜRÜR
        database.close();
        if(result==-1)   // -1 İSE HATA YOK.
            return false;
        else
            return true;
    ...
    public String findUser(String userName) {
        ...
        Cursor cursor=database.rawQuery(query,null);    // CURSOR CLASSI TÜM KAYITLARIN SAYISINI BULMAK İÇİN
        if(cursor.moveToFirst()) {    // TRUE İSE BOŞ DEĞİLDİR
            database.close();
            return cursor.getString(4);   // 4.SÜTUN YANİ PASSWORD
        }else{
            database.close();
            return "not found";
        }
